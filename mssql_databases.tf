...
module "mssql_databases" {
  source   = "./databases/mssql_database"
  ...
  settings            = each.value
  server_id           = can(each.value.server_id) ? each.value.server_id : local.combined_objects_mssql_servers[each.value.mssql_server_key].id
  server_name         = can(each.value.server_name) ? each.value.server_name : local.combined_objects_mssql_servers[each.value.mssql_server_key].name
  elastic_pool_id     = can(each.value.elastic_pool_id) || can(each.value.elastic_pool_key) == false ? try(each.value.elastic_pool_id, null) : local.combined_objects_mssql_elastic_pools[each.value.elastic_pool_key].id
