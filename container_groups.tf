module "container_groups" {
  source     = "./compute/container_group"
...
module "network_profiles" {
  source   = "./networking/network_profile"
  ...
  global_settings = local.global_settings
  settings        = each.value
  resource_group  = local.combined_objects_resource_groups[try(each.value.resource_group_key, each.value.resource_group.key)]
