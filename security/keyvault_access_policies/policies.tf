...
module "azuread_group" {
  ...
  access_policy = each.value
  tenant_id     = var.client_config.tenant_id
  object_id     = try(var.azuread_groups[each.value.azuread_group_key].id, null)
}
...
module "mssql_managed_instance" {
  ...
  access_policy = each.value
  tenant_id     = var.client_config.tenant_id
  object_id     = try(var.resources.mssql_managed_instances[each.value.mssql_managed_instance_key].principal_id, null)
}
...
module "mssql_managed_instances_secondary" {
  ...
  access_policy = each.value
  tenant_id     = var.client_config.tenant_id
  object_id     = try(var.resources.mssql_managed_instances_secondary[each.value.mssql_managed_instance_secondary_key].principal_id, null)
}
