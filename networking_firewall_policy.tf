...
module "azurerm_firewall_policies" {
  source = "./networking/firewall_policies"
  for_each = {
  ...
  resource_group = can(each.value.resource_group.id) ? each.value.resource_group.id : local.combined_objects_resource_groups[try(each.value.resource_group_key, each.value.resource_group.key)]
}
...
module "azurerm_firewall_policies_child" {
  ...
  base_policy_id = can(each.value.base_policy.id) ? each.value.base_policy.id : local.combined_objects_azurerm_firewall_policies[each.value.base_policy.key].id
...
module "azurerm_firewall_policy_rule_collection_groups" {
  source   = "./networking/firewall_policy_rule_collection_groups"
  ...
  firewall_policy_id = can(each.value.firewall_policy_id) || can(module.azurerm_firewall_policies_child[each.value.firewall_policy.key]) ? try(each.value.firewall_policy_id, module.azurerm_firewall_policies_child[each.value.firewall_policy.key].id) : local.combined_objects_azurerm_firewall_policies[try(each.value.firewall_policy_key, each.value.firewall_policy.key)].id
