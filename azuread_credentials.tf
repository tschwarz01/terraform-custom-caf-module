...
module "azuread_credentials" {
  ...
  resources = {
    application = {
      id             = try(local.combined_objects_azuread_applications[each.value.azuread_application.key].id, {})
      application_id = try(local.combined_objects_azuread_applications[each.value.azuread_application.key].application_id, {})
    }
  }
}
