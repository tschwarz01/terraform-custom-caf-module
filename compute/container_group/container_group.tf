...
data "azurerm_key_vault_secret" "image_registry_credential_password" {
  ...
  key_vault_id = try(var.combined_resources.keyvaults[each.value.keyvault_key].id, null)
...
resource "azurerm_container_group" "acg" {
  ...
  network_profile_id  = try(var.combined_resources.network_profiles[var.settings.network_profile.key].id, null)
