...
resource "azurerm_network_interface" "nic" {
  ...
  ip_configuration {
    ...
    public_ip_address_id          = can(each.value.public_address_id) || can(each.value.public_ip_address_key) == false ? try(each.value.public_address_id, null) : var.public_ip_addresses[each.value.public_ip_address_key].id
  }
  ...
  dynamic "ip_configuration" {
    ...
    content {
      ...
      subnet_id                     = can(ip_configuration.value.subnet_id) ? ip_configuration.value.subnet_id : var.vnets[ip_configuration.value.vnet_key].subnets[ip_configuration.value.subnet_key].id
      ...
      public_ip_address_id          = can(ip_configuration.value.public_address_id) || can(ip_configuration.value.public_ip_address_key) == false ? try(ip_configuration.value.public_address_id, null) : var.public_ip_addresses[ip_configuration.value.public_ip_address_key].id
    }
  }
}
