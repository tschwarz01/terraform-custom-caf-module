...
resource "azurerm_virtual_machine" "vm" {
  ...
  availability_set_id = can(each.value.availability_set) == false || can(each.value.availability_set.id) || can(each.value.availability_set_id) ? try(each.value.availability_set.id, each.value.availability_set_id, null) : var.availability_sets[try(each.value.availability_set_key, each.value.availability_set.key)].id
  ...
  dynamic "storage_image_reference" {
    ...
    content {
      ...
      id        = try(each.value.storage_image_reference.custom_image_id, var.custom_image_ids[each.value.custom_image_key].id, null)
    }
  }
