  ...
  dynamic "key_vault_credential" {
    ...
    content {
      ...
      key_vault_url = coalesce(
        try(var.keyvaults[each.value.mssql_settings.sql_authentication.keyvault_credential.keyvault_key].vault_uri, null)
      )
...
data "azurerm_storage_account" "mssqlvm_backup_sa" {
  ...
  name = coalesce(
    try(var.storage_accounts[each.value.mssql_settings.auto_backup.storage_account.key].name, null)
  )
  resource_group_name = coalesce(
    try(var.storage_accounts[each.value.mssql_settings.auto_backup.storage_account.key].resource_group_name, null)
  )
}
...
data "external" "sql_username" {
  ...
  program = [
    ...
    format(
      ...
      try(var.keyvaults[each.value.mssql_settings.sql_authentication.sql_credential.keyvault_key].name, null)
    )
  ]
}
...
data "external" "backup_encryption_password" {
  ...
  program = [
    ...
    format(
      ...
      try(var.keyvaults[each.value.mssql_settings.auto_backup.encryption_password.keyvault_key].name, null)
    )
  ]
}
...
data "external" "sp_client_id" {
  ...
  program = [
    ...
    format(
      ...
      try(var.keyvaults[each.value.mssql_settings.sql_authentication.keyvault_credential.service_principal_secrets.keyvault_key].name, null)
    )
  ]
}
...
resource "azurerm_key_vault_secret" "sql_admin_password" {
  ...
  key_vault_id = coalesce(
    try(var.keyvaults[each.value.mssql_settings.sql_authentication.sql_credential.keyvault_key].id, null)
  )
...
resource "azurerm_key_vault_secret" "backup_encryption_password" {
  ...
  key_vault_id = coalesce(
    try(var.keyvaults[each.value.mssql_settings.auto_backup.encryption_password.keyvault_key].id, null)
  )
