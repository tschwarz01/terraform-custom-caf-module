...
resource "azurerm_windows_virtual_machine" "vm" {
  ...
  availability_set_id          = can(each.value.availability_set_key) || can(each.value.availability_set.key) ? var.availability_sets[try(each.value.availability_set_key, each.value.availability_set.key)].id : try(each.value.availability_set.id, each.value.availability_set_id, null)
  ...
  dedicated_host_id = can(each.value.dedicated_host.key) ? var.dedicated_hosts[each.value.dedicated_host.key].id : try(each.value.dedicated_host.id, null)
