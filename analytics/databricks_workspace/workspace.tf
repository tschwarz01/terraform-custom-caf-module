...
resource "azurerm_databricks_workspace" "ws" {
  ...
  dynamic "custom_parameters" {
    ...
    content {
      ...
      #   try(var.aml.id, null),
      #   try(var.aml[var.settings.custom_parameters.machine_learning.key].id, null),
      #   ),
      #   null
      # )

      virtual_network_id  = can(var.settings.custom_parameters.virtual_network_id) || can(var.settings.custom_parameters.vnet_key) == false ? try(var.settings.custom_parameters.virtual_network_id, null) : var.vnets[var.settings.custom_parameters.vnet_key].id
      public_subnet_name  = can(var.settings.custom_parameters.public_subnet_name) || can(var.settings.custom_parameters.vnet_key) == false ? try(var.settings.custom_parameters.public_subnet_name, null) : var.vnets[var.settings.custom_parameters.vnet_key].subnets[var.settings.custom_parameters.public_subnet_key].name
      private_subnet_name = can(var.settings.custom_parameters.private_subnet_name) || can(var.settings.custom_parameters.private_subnet_key) == false ? try(var.settings.custom_parameters.private_subnet_name, null) : var.vnets[var.settings.custom_parameters.vnet_key].subnets[var.settings.custom_parameters.private_subnet_key].name

      #the NSG-association ID is the subnet-id, so it can be simplified:
      public_subnet_network_security_group_association_id = can(var.settings.custom_parameters.public_subnet_network_security_group_association_id) || can(var.settings.custom_parameters.public_subnet_key) == false ? try(var.settings.custom_parameters.public_subnet_network_security_group_association_id, null) : var.vnets[var.settings.custom_parameters.vnet_key].subnets[var.settings.custom_parameters.public_subnet_key].id
      #the NSG-association ID is the subnet-id, so it can be simplified:
      private_subnet_network_security_group_association_id = can(var.settings.custom_parameters.private_subnet_network_security_group_association_id) || can(var.settings.custom_parameters.public_subnet_key) == false ? try(var.settings.custom_parameters.private_subnet_network_security_group_association_id, null) : var.vnets[var.settings.custom_parameters.vnet_key].subnets[var.settings.custom_parameters.private_subnet_key].id
    }
  }
}
