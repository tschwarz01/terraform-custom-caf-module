module "disk_encryption_sets" {
  source   = "./security/disk_encryption_set"
  ...
  key_vault_key_id    = local.combined_objects_keyvault_keys[try(each.value.key_vault_key_key, each.value.key_vault_key.key)].id
  keyvault_id         = local.combined_objects_keyvaults[each.value.keyvault.key].id
}
